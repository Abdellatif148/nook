...
export const LoginPage: React.FC = () => {
  ...
  const handleOwnerLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!cafeData || !cafeData.setup_complete) {
        navigate('/wizard')
      } else {
        navigate('/dashboard')
      }
  ...
  const handleVerifyCafe = async () => {
    ...
    try {
      const { data: cafeData, error } = await supabase
        .from('cafes')
        .select('id, name')
        .eq('invite_code', cafeCode)
        .single()

      if (error || !cafeData) throw new Error('Code incorrect')

      // Load active staff for this cafe
    ...
    try {
      ...
      if (hashed === selectedStaff.pin_hash) {
        ...
        const { data: cafeData } = await supabase
          .from('cafes')
          .select('*')
          .eq('id', selectedStaff.cafe_id)
          .single()

        const sessionData = {
          type: 'staff',
          staff_id: selectedStaff.id,
          cafe_id: selectedStaff.cafe_id,
          name: selectedStaff.name,
          permissions: selectedStaff.permissions,
          expires_at: new Date(Date.now() + 12 * 60 * 60 * 1000).toISOString()
        }

        localStorage.setItem('nook_staff_session', JSON.stringify(sessionData))
        ...
        {step === 1 && (
          <div className="p-1 bg-surface2 border border-border rounded-btn flex mb-8">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-btn text-sm font-medium transition-all ${
        ...
        <AnimatePresence mode="wait">
          {role === 'owner' ? (
            ...
            >
              ...
              {error && (
                <p className="text-xs text-error text-center bg-error-dim py-2 rounded-lg border border-error/10 animate-shake">
                  {error}
                </p>
              )}
              ...
              <div className="text-center pt-2">
                <span className="text-[13px] text-text3">{t('auth.no_account')} </span>
                <Link to="/register" className="text-[13px] text-accent2 hover:underline font-semibold transition-all">
              ...
              {step === 1 ? (
                <div className="space-y-6">
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between gap-2">
                      <Input
                        placeholder="000000"
                        className="text-center font-mono tracking-[0.4em] text-xl h-14"
                        maxLength={6}
                        value={cafeCode}
                        onChange={(e) => setCafeCode(e.target.value.replace(/[^0-9]/g, '').slice(0, 6))}
                      />
                    </div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-3">
                    <button
                      onClick={() => setStep(1)}
                      className="p-2 -ml-2 text-text3 hover:text-text transition-colors"
                    >
                      <ArrowLeft className="w-5 h-5" />
                    </button>
                    <span className="text-[14px] font-bold text-text truncate">Nook Cafe</span>
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-[11px] font-semibold text-text3 uppercase tracking-wider pl-1">{t('auth.your_name')}</label>
                    <select
                      className="w-full bg-black/25 border border-border rounded-btn px-3.5 py-2.5 text-text outline-none focus:border-accent min-h-[44px]"
